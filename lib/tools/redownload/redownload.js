...
var Redownload = function() {
    ...
    function recheckAllFiles(data) {
        ...
        var redownloadList = requestsList.map(function(entry) {
            return function(callback) {
                ...
                .then(function(entry) {
                    return fontAnalyzer.analyzeFont(entry, data.toolsResults.phantomas.offenders.differentCharacters[0]);
                })
    ...
    function listHeavyFonts(requests) {
        ...
        requests.forEach(function(req) {
            if (req.isWebFont && req.weightCheck.bodySize > MAX_FONT_WEIGHT) {
                list.push({
                    url: req.url,
                    size: req.weightCheck.bodySize
                });
